...
public class GameService
{
    ...
    public async Task<GameDto> CreateGame(int hostId)
    {
        ...
        if (host == null) throw new ArgumentException("Player not found");
    ...
    public async Task<GameDto> JoinGame(int gameId, int guestId)
    {
        ...
        if (efGame == null) throw new ArgumentException("Game not found");

        var guest = await _playerRepository.GetByIdAsync(guestId);
        if (guest == null) throw new ArgumentException("Player not found");

        if (efGame.HostId == guestId) throw new ArgumentException("Host cannot join his own game");
    ...
    public async Task<GameDto> PlayTurn(int gameId, int playerId, int column)
    {
        ...
        if (efGame.Status != EFGame.Statuses.InProgress) throw new InvalidOperationException("Game is not in progress");

        if (efGame.Grid == null) throw new InvalidOperationException("Grid is not initialized for this game.");

        var efPlayer = await _playerRepository.GetByIdAsync(playerId);
        if (efPlayer == null) throw new ArgumentException("Player not found");
