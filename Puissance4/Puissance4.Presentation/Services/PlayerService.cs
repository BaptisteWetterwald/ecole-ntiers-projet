...
public class PlayerService
{
    ...
    {
        var response =
            await _httpClient.PostAsJsonAsync("players/login", new { Username = username, Password = password });
        response.EnsureSuccessStatusCode();
    ...
    {
        var response =
            await _httpClient.PostAsJsonAsync($"games/{gameId}/players/{playerId}/play", new { Column = column });
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task<int> GetPlayerIdAsync()
    {
        ...
        if (string.IsNullOrEmpty(playerIdString)) throw new Exception("PlayerId not found in token");

        if (!int.TryParse(playerIdString, out var playerId)) throw new Exception("Invalid PlayerId format in token");
